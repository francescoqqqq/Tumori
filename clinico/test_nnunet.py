...
def test_nnunet_model(dataset_id=500, config='2d', fold=0, results_dir='risultati', 
    ...
    # Setup paths (relative to clinico directory)
    ...
    base_dir = os.path.dirname(os.path.abspath(__file__))
    hybrid_splits_file = os.path.join(base_dir, 'hybrid_splits.json')
    ...
    for case_id in tqdm(test_cases, desc="Processing cases"):
        try:
            ...
            # Calculate metrics
            # Use VALUES (integers) not KEYS (strings) for class IDs
            class_ids = list(class_names.values())
            metrics = calculate_metrics(pred_data, gt_data, class_labels=class_ids)
